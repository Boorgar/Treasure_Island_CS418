...
class treasureIsland:
    ...
    def pirateFindPath(self):
        ...
        def isValid(x, y, N, flag):
            if flag == 1:
                return (0 <= x < N) and (0 <= y < N) and (self.map[x][y] != 0) and (self.terrain[x][y] != 77) 
            return (0 <= x < N) and (0 <= y < N) and (self.map[x][y] != 0) and (self.terrain[x][y] != 80) and (self.terrain[x][y] != 77)
        ...
        def findPath(start, goal, move, N):
            path = []
            val = 1
            visited = set()
            ...
            y = start[1]
            g = 0
            f = g + (abs(x - goal[0]) + abs(y - goal[1]))
            minList = [f,g,x,y]

            startNode = Node(x,y, None)
            q = deque()
            q.append(startNode)
            flag = 1
            while q:
                ...
                for i in range(4):
                    ...
                    key = (x2,y2)
                    #print(key)
                    if isValid(x2,y2,N, flag) and key not in visited:
                ...
                q.remove(Node(x,y, None))

                next = Node(x, y, curr)
                q.append(next)
                
                #flag just for make sure that pirate can move out of prison at first move and can not pass through prison
                flag = flag + 1
            return
        ...
        goal = self.treasurePos
        #print(start, goal)
        path =  findPath(start, goal, move, N)
        print(path)
